...
interface CategoryDummyListItem {
  id: string;
...
interface OutputCategoryListItem {
  categoryIndex: number;
  id: string;
  name?: string;
}
type FunctionOutput = OutputCategoryListItem[];

const fetchCategoryListPayloadResolver3 = ({ categories }: FunctionArgs): FunctionOutput => {
  const mappedCategories = categories.map(
    ({ _id: id, name }, categoryIndex: number): OutputCategoryListItem => ({
      categoryIndex,
      id,
      name,
    }),
...
// const fetchCategoryListPayloadResolver3 = ({
//   categories,
// }: FunctionArgs): FunctionOutput => {
//   return categories
//     .map(({ _id: id, name }, categoryIndex: number) => ({
//       categoryIndex,
//       id,
//       name,
//     }))
